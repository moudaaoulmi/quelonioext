...
public class FmpActionBarContributor
	...
	protected IAction showConstraintsViewAction =
		new Action(fmpEditPlugin.INSTANCE.getString("_UI_ShowConstraintsView_menu_item")) {
			public void run() {
				try {
					ConstraintsView2 constraintsView = (ConstraintsView2) getPage().showView(ConstraintsView2.VIEW_ID);
					constraintsView.updateInput(selectionProvider.getSelection());
				}
				catch (PartInitException exception) {
		...
		if (selection instanceof IStructuredSelection && ((IStructuredSelection)selection).size() == 1) {
			...
			if (object instanceof Reference) {
				...
				if (RoleQuery.INSTANCE.getNodeType(feature) == RoleQuery.ROOT_FEATURE) {
					...
					AddConstraintAction cAction = new AddConstraintAction();
					cAction.updateSelection((IStructuredSelection)selection);
					additionalActions.add(cAction);
