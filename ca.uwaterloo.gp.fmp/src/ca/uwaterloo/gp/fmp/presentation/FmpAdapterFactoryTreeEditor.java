...
import ca.uwaterloo.gp.fmp.provider.command.ChangeFeatureStateCommand;
...
public class FmpAdapterFactoryTreeEditor extends ExtendedTreeEditor {
	...
	protected void checkItem(TreeItem treeItem, boolean controlKeyPressed) {
		...
		if (object instanceof Clonable)  {
			...
			//Elder
			/*Feature selModelRoot = ModelNavigation.INSTANCE.navigateToModelRoot((Node)owner);

			int selRole = RoleQuery.INSTANCE.getNodeType((Node)owner);
			Feature configRoot = (selRole == RoleQuery.ROOT_FEATURE) ? (Feature)owner :	ModelNavigation.INSTANCE.navigateToRootFeature((Node)owner);				
			int location = RoleQuery.INSTANCE.getLocationType(configRoot);

			Feature selConfig = null;

			if (location == RoleQuery.CONFIGURATION	|| location == RoleQuery.CONFIGURATION_PROPERTIES)
				selConfig = configRoot;

			BDDBuilder builder = ((NodeImpl)selModelRoot).getIdTable().getBDDBuilder();
			Feature sel = (Feature)owner;
			for ( int x = 0; x < selModelRoot.getConstraints().size(); x++ ) {
				Constraint c = (Constraint)selModelRoot.getConstraints().get(x);
				CompoundCommand changeState = new ResolveConstraintCommand(editingDomain,builder,selConfig,selModelRoot,c);

				if ( changeState.canExecute() ) {
					editingDomain.getCommandStack().execute(changeState);
				}
			}*/
			treeViewer.refresh();
		}
