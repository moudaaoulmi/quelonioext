...
public class ModelManipulation
{
	...
	/**
	 * Called by ConfigureFeatureCommand
	 * @param node
	 * @return
	 */
	...
	/**
	 * Called by cnofigureTree(Node).
	 * STEVEN: can be called multiple times per node for some reason.
	 * @param node
	 * @return
	 */
...
//							String newId = NodeIdDictionary.INSTANCE.getIdForName((String) newValue);
							String newId = fmpEditor.getIdTable().createIdentifier((String)newValue);
					...
					if (typedValue != null)  {
						...
						if (valueType != ValueType.NONE_LITERAL)  {
							...
							if (RoleQuery.INSTANCE.getPropertyRole((Node)owner) == RoleQuery.NAME) {
								...
								//FIXME Steven: "hack" to sanitize id
								if (RoleQuery.INSTANCE.getPropertyRole((Node)owner) == RoleQuery.ID
										&& RoleQuery.INSTANCE.getLocationType((Node)owner) != RoleQuery.CONFIGURATION
										&& RoleQuery.INSTANCE.getLocationType((Node)owner) != RoleQuery.CONFIGURATION_PROPERTIES) {
									newValue = fmpEditor.getIdTable().createIdentifier((String)newValue);
								}
