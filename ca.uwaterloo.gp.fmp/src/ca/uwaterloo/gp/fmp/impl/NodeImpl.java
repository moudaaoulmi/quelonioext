...
public abstract class NodeImpl extends EObjectImpl implements Node {
	...
	/**
	 * This listener listens for changes in the feature hierarchy and sets the
	 * IdTable attribute in the FeatureImpl accordingly.
	 * FIXME this is currently in FeatureImpl since I don't want to touch 
	 * fmp.ecore for now...
	 * @author Steven She <shshe@uwaterloo.ca>
	 */
	protected class NewChildListener extends EContentAdapter {
		protected void setIdTable(NodeImpl child) {
			child.setIdTable(getIdTable());
		}
		
		@Override
		public void notifyChanged(Notification notification) {
			if (notification.getFeatureID(FmpPackage.class)
					== FmpPackage.NODE__CHILDREN ||
					notification.getFeatureID(FmpPackage.class)
					== FmpPackage.FEATURE__CONFS) {
				NodeImpl parent = (NodeImpl)notification.getNotifier();
				switch (notification.getEventType()) {
				case Notification.ADD:
					//TODO move to Node
 					NodeImpl child = (NodeImpl)notification.getNewValue();
					setIdTable(child);
					break;
				case Notification.ADD_MANY:
					List<NodeImpl> newChildren = (List<NodeImpl>) notification.getNewValue();
					for (NodeImpl f : newChildren) {
						setIdTable(f);
					}
					break;
				}
	...
	protected IdTable idTable = null; 
	public IdTable getIdTable() {
		if (idTable == null) {
//			fmpEditPlugin.INSTANCE.getPlugin().getLog().log(new Status(IStatus.WARNING,
//				fmpEditPlugin.PLUGIN_ID, "Id Table is empty for Node " + getId() + ", creating new Id Table"));
			setIdTable(new IdTable());
		}
		return idTable;
	}
	 ...
	 * <!-- end-user-doc -->
	 * @generated
	 * @see NewChildListener
	 */
	protected NodeImpl() {
		super();
		eAdapters().add(new NewChildListener());
	}
...
//			newId = fmpEditor.getIdTable().createIdentifier(newId);
//		}
