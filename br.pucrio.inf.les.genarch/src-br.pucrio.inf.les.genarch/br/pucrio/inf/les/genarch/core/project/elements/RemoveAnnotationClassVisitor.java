...
import br.pucrio.inf.les.genarch.core.annotations.Variability;

public class RemoveAnnotationClassVisitor extends ASTVisitor {
	...
	if ( node.getName().getIdentifier().equals("Feature") || node.getName().getIdentifier().equals("Variability") ) {
	    node.delete();
	}
	return this.visitNode(node);
    }
	...
	if ( node.getTypeName().getFullyQualifiedName().equals("Feature") || node.getTypeName().getFullyQualifiedName().equals("Variability")) {
